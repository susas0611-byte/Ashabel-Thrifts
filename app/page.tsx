      ...
      <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-slate-200 shadow-xs">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3 cursor-pointer" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
            <img 
              src="/image/e3bd8a59-60eb-4ba9-8ed9-a7fdd8116f98.jpg" 
              alt="Ashabel Logo" 
          ...
          <nav className="hidden md:flex items-center gap-8 font-medium text-slate-600 text-sm">
            <a href="#shop" className="hover:text-teal-600 transition-colors">Catalog</a>
            <a href="#features" className="hover:text-teal-600 transition-colors">Why Buy From Us</a>
            <a href="#footer" className="hover:text-teal-600 transition-colors">Contact</a>
          </nav>

          {/* Search & Cart Actions */}
          <div className="flex items-center gap-4">
            <div className="hidden lg:flex items-center bg-slate-100 rounded-full px-4 py-2 border border-slate-200 focus-within:border-teal-500 transition-all">
              <Search className="w-4 h-4 text-slate-400 mr-2" />
              <input 
                type="text" 
                placeholder="Search size 38, 39, 42..." 
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="bg-transparent text-sm outline-none w-48 text-slate-800 placeholder:text-slate-400"
              />
            </div>

            <button 
              onClick={() => setIsCartOpen(true)}
              className="relative bg-teal-600 text-white p-3 rounded-full hover:bg-teal-700 transition-all shadow-md shadow-teal-600/20 group"
            >
              <ShoppingBag className="w-5 h-5 group-hover:scale-110 transition-transform" />
              {totalItems > 0 && (
                <span className="absolute -top-1 -right-1 bg-amber-500 text-slate-950 text-xs font-bold w-5 h-5 rounded-full flex items-center justify-center border-2 border-white shadow">
                  {totalItems}
                </span>
              )}
            </button>
        ...
        <AnimatePresence>
          {isMobileMenuOpen && (
            <motion.div 
              ...
              className="md:hidden bg-white border-b border-slate-200 px-6 py-4 flex flex-col gap-4 shadow-lg"
            >
              ...
              <a href="#shop" onClick={() => setIsMobileMenuOpen(false)} className="font-medium text-slate-700 py-2">Catalog</a>
              <a href="#features" onClick={() => setIsMobileMenuOpen(false)} className="font-medium text-slate-700 py-2">Why Buy From Us</a>
              <a href="#footer" onClick={() => setIsMobileMenuOpen(false)} className="font-medium text-slate-700 py-2">Contact</a>
            </motion.div>
          )}
        </AnimatePresence>
      </header>
